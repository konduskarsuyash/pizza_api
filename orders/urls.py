...
urlpatterns = [
    ...
    path('update-status/<int:order_id>/',views.UpdateOrderStatus.as_view(), name='order-update-status'),
    path('user/<int:user_id>/orders/',views.UserOrdersView.as_view(), name='users_orders'),
    path('user/<int:user_id>/order/<int:order_id>/',views.UserOrderDetail.as_view(), name='users_order_detail')
]
